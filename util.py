...
def getMA(data, N, j=0):
    ...
    :return: [MAn]
    '''
    return sum(d[4] for d in data[-1 - N + j: -1 + j]) / float(N)
...
def getBoll(data, N=20, K=2, j=0):
    ...
    '''
    MB = getMA(data, N)
    delta = K * np.std([d[4] for d in data[-1 - N + j: -1 + j]])
    UP = MB + delta
    LB = MB - delta
    PB = (data[-1 + j][4] - LB) / (UP - LB)
    BW = (UP - LB) / MB
...
def macdjincha(data):
    ...
    # 金叉或者死叉
    if df['DIF'][-1] > df['DEM'][-1] and df['DIF'][-2] < df['DEM'][-2] and data[-1][4] < MB:
        return 'up'
    elif (df['DIF'][-1] < df['DEM'][-1] and df['DIF'][
        -2] > df['DEM'][-2] and data[-1][4] > MB):
        return 'down'
